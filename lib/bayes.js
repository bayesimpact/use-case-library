...
(function () {
  ...
  /* ---------------- boot ---------------- */
  if ("scrollRestoration" in history) history.scrollRestoration = "manual";   // always open on the title slide
  Believe.start();
  window.addEventListener("load", function () {
    // always open on the title slide (the scroll observer pollutes the hash, so we ignore it on load)
    cur = 0;
    try { history.replaceState(null, "", location.pathname + location.search); } catch (x) {}
    slides[0].scrollIntoView({ behavior: "auto" });
    slides[0].classList.add("show"); chrome(); enter(0);
  });
})();
